...
- 相比于稳定前向输入分布，反向传播时mean和variance计算引入的梯度更有用，LN可以稳定反向传播时loss对输入的梯度；
- LN特别适合处理变长数据，因为是对channel维度（hidden维度）做操作，和句子长度和batch大小无关；

![image-20220914225541398](%E6%B7%B1%E5%BA%A6%E5%AD%A6%E4%B9%A0.assets/image-20220914225541398.png)

- **由于CV与NLP数据特性不同**（NLP数据在前向和反向传播中，batch统计量与梯度不稳定），因此Transformer使用LayerNorm；
